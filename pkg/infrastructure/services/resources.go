...
func (s *serviceDeployer) applyKafkaConfigurations(deployment *appsv1.Deployment, instance v1alpha1.KogitoService) error {
	URI, err := getKafkaServerURI(instance.GetSpec().(v1alpha1.KafkaAware).GetKafkaProperties(), s.definition.Namespace, s.client)
	if err != nil {
		return err
	}
...
func (s *serviceDeployer) createKafkaTopics(instance v1alpha1.KogitoService) (topics []resource.KubernetesResource, err error) {
	...
	kafkaName, kafkaReplicas, err := getKafkaServerReplicas(instance.GetSpec().(v1alpha1.KafkaAware).GetKafkaProperties(), s.definition.Namespace, s.client)
	if err != nil {
		return
	...
	for _, topic := range s.definition.KafkaTopics {
		kafkaTopic := newKafkaTopic(topic.TopicName, kafkaName, kafkaReplicas, s.definition.Namespace)
